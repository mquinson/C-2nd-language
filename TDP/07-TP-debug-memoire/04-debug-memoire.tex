...
déclenchée par erreur et c'est très pratique! G.H.
  ...
  \noindent\begin{minipage}[b]{.6\linewidth}
    ...
    \textbf{Pour le compiler}, il convient d'utiliser la commande \run{gcc
      -Wall -g -o boucle boucle.c}. L'option \texttt{-Wall} demande
    ...
    nécessaire à \cmd{gdb} (et autres debuggers).
...
\begin{Exercice}\textbf{le debugger GNU: gdb (utilisation avec les fonctions)}
  ...
  processus en cours d'exécution, et bien d'autres encore. \run{info gdb} pour
  les détails.
